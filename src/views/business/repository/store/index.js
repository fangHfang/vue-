...
import {
  ...
  infoDeleteWhiteList,
  infoQueryVisibility,
  infoCreateVisibility,
  infoDeleteVisibility
} from "@/api/business/repository/info";
export default {
  ...
  "methods": {
    ...
    visibleChangeAll(val) {
      ...
      infoCreateVisibility(params).then((res) => {
        if (res.code === 200) {
    ...
    async getRemoteStoreTableData() {
      ...
      const res = await infoQueryWhiteList(this.$jsonFormat(params));
      if (res.code === 200) {
    ...
    listItemVisibilityPage() {
      ...
      infoQueryVisibility(params).then(res => {
        if (res.code === 200) {
    ...
    delVisibility(row) {
      this.$confirm("是否确定要删除所选可见性?", "删除可见性", {
        ...
        infoDeleteVisibility(params).then((res) => {
          if (res.code === 200) {
    ...
    delStore() {
      ...
      this.$confirm("是否确定要删除所选门店?", "删除门店", {
        ...
        infoDeleteWhiteList(params).then((res) => {
          if (res.code === 200) {
            this.$message.success("删除成功");
            this.getRemoteStoreTableData();
          } else {
    ...
    getSelectStore(list) {
      ...
      infoCreateWhiteList(params).then((res) => {
        if (res.code === 200) {
