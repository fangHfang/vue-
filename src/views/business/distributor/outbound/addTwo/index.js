...
export default {
  ...
  "methods": {
     ...
     */
    back() {
      this.$router.push({ "path": "/business/distributor/outbound/list" });
    },
    ...
    saveDealerTwo() {
      ...
      this.form.productList.some(x => {
        ...
        if (parseInt(x.count) <= 0) {
          canSave = false;
          return this.$message.warning("出库数量必须大于0");
        }
      });
      const params = {
    ...
    changeInput(count, index) {
      console.log(this.form.productList[index].count);
      this.form.productList[index].count = count;
