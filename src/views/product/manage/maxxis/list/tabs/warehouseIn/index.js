...
export default {
  ...
  data() {
    return {
      ...
      "exportUrl": ITEM_URL + "/admin/itemInfo/exportDealerPrice",
      "templateUrl": ITEM_URL + "/admin/itemInfo/download/template"
    };
  },
  "computed": {
    permission() {
    ...
    // 导出入参
    exportGetData() {
      return this.$jsonFormat(this.searchData);
    }
  },
  mounted() {
    // 封装下拉框
  ...
  "methods": {
     ...
     */
    toAddProduct() {
      this.$router.push({ "path": "/product/manage/maxxis/add", "query": { "r": this.$getRandomValue() } });
    },
    /**
     * 上架商品
     */
     ...
     */
    toDetail(row) {
      this.$router.push({ "path": "/product/manage/maxxis/detail/" + row.itemNo });
    },
    /**
     * 编辑
     */
    toEdit(row) {
      this.$router.push({ "path": "/product/manage/maxxis/edit/" + row.itemNo });
    },
    /**
     * 查看价格弹出层
     * @param row
