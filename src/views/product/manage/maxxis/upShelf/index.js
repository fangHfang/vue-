  ...
  "methods": {
      ...
      listSearchRbRule("type=0").then(res => {
        if (res.code === 200) {
          res.data.map(element => {
            if (element.type === 0 && element.status === 1) {
              const options = { "label": element.ruleName, "value": element.ruleNo };
              this.form.rebateRule.options.push(options);
            }
          });
        } else {
      ...
      listSearchItRule("type=0").then(res => {
        if (res.code === 200) {
          res.data.map(element => {
            if (element.type === 0 && element.status === 1) {
              const options = { "label": element.ruleName, "value": element.ruleNo };
              this.form.integralRole.options.push(options);
            }
          });
        } else {
    ...
    applySaleStepOne() {
      this.$refs.formRef.validate((valid) => {
        if (valid) {
          this.doSave().then(res => {}).catch(res => {
    ...
    toPutOnTwo() {
      if (this.detail) {
        this.$router.push({ "path": "/product/manage/maxxis/upShelfTwo", "query": { "itemNo": this.itemNo, "detailInfo": true } });
        return;
      }
      if (this.saved) {
        this.$router.push({ "path": "/product/manage/maxxis/upShelfTwo", "query": { "itemNo": this.itemNo } });
        return;
      }
      this.$refs.formRef.validate((valid) => {
        if (valid) {
