  ...
  <div class="page">
    <div class="page-content" style="padding-bottom: 0;">
      ...
      <el-form :model="form" :rules="rules" ref="formRef" label-width="90px" label-position="left">
        <div class="item-box" v-if="false">
          <h4 class="item-title">
            返利规则设置
            <el-switch :disabled="detail" v-model="form.rebateRule.status" style="margin-left:10px;" />
            <span class="normal">（单选）</span>
          </h4>
          <div class="del-form">
            <el-form-item label="返利规则" prop="rebateRule.value">
              <el-select
                :disabled="!form.rebateRule.status || detail"
                v-model="form.rebateRule.value"
                filterable
                placeholder="请选择返利规则"
                style="width:200px;"
              >
                <el-option
                  v-for="item in form.rebateRule.options"
                  :key="item.value"
                  :label="item.label"
                  :value="item.value"
                >
        ...
        <div class="item-box" v-if="false">
          <h4 class="item-title">
            积分规则设置
            <el-switch :disabled="detail" v-model="form.integralRole.status" style="margin-left:10px;" />
            <span class="normal">（单选）</span>
          </h4>
          <div class="del-form">
            <el-form-item label="积分规则" prop="integralRole.value">
              <el-select
                :disabled="!form.integralRole.status || detail"
                v-model="form.integralRole.value"
                placeholder="请选择积分规则"
                filterable
                style="width:200px;"
              >
                <el-option
                  v-for="item in form.integralRole.options"
                  :key="item.value"
                  :label="item.label"
                  :value="item.value"
                >
        ...
        <div class="item-box">
          <h4 class="item-title spec">
            <span>选择经销商</span>
            <div class="actionbar">
              <el-button type="primary" :disabled="detail" @click="setDealer">设置</el-button>
              <el-button type="primary" :disabled="detail" @click="delDealer">移除</el-button>
            </div>
          </h4>
    ...
    <div class="table-main">
      <div class="btn-box" style="padding-bottom: 30px;">
        <el-button type="primary" v-if="!detail" @click="btnLoading = true;$throttle(applySaleStepOne)" :loading='btnLoading'>暂存</el-button>
        <el-button type="primary" @click="btnLoading = true;$throttle(toPutOnTwo)" :loading='btnLoading'>下一步</el-button>
        <el-button @click="back">放弃设置</el-button>
      </div>
    </div>
    <dialogAddDealer ref="dealerRef" mark="itemUpShelf" :dialogVisible.sync="dialogDealerVisible" :itemNo="itemNo" @selectData="getSelectDealer"/>
