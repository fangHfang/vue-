...
export default {
  ...
  "methods": {
     ...
     * @param specNo
     */
    specTemplateChange(specNo) {
      if (!specNo) {
        this.specTemplateFields = [];
        return;
      }
      const selectedSpec = this.specTemplateOptions.find(specTemplate => { return specTemplate.specNo === specNo; });
      if (!selectedSpec) {
        this.specTemplateFields = [];
        return;
      }
      const specTemplateFields = this._.split(selectedSpec.specFiled, "|");
      if (this._.size(specTemplateFields) <= 0) {
        this.specTemplateFields = [];
        return;
      }
      const result = [];
      // 将规格明细分为4个一组，便于页面展示
      for (let i = 0, len = specTemplateFields.length;i < len;i += 4) {
        result.push(specTemplateFields.slice(i, i + 4));
      }
      result.forEach(row => {
        row.forEach(cell => {
          this.rules[`specDetail.${cell}`] = [
            { "required": true, "message": `请输入${cell}`, "trigger": "blur" }
          ];
        });
      });
      this.specTemplateFields = result;
    },
    /**
     * 获取商品详情
     */
    getGoodsDetail() {
      getItemInfoDetail(this.itemNo).then(res => {
        if (res.code === 200) {
          ...
          this.addForm.specNo = res.data.specNo || "";
          ...
          this.specTemplateChange(this.addForm.specNo);
          try {
            const specDetail = res.data.specDetail.replace(/ /g, ":").replace(/^/g, "{\"").replace(/$/g, "\"}").replace(/\B:/g, "\":\"").replace(/,\B/g, "\",\"");
            this.addForm.specDetail = res.data.specDetail && JSON.parse(specDetail);
          } catch {
            this.addForm.specDetail = {};
          }
        } else {
    ...
    tableProductList() {
      ...
      this.detailLoading = true;
      pageSearchProductList(this.$jsonFormat(params)).then(res => {
        if (res.code === 200) {
    ...
    toPutOn() {
      if (this.identity === "add") {
        if (!this.itemNo) {
          this.$refs.addForm.validate(async (valid) => {
            if (valid) {
              this.doSave().then((res) => {
                if (res) {
                  this.$router.push({ "path": "/product/manage/nomaxxis/upShelf", "query": { "itemNo": res } });
                }
              }).catch(res => {
          ...
          this.$router.push({ "path": "/product/manage/nomaxxis/upShelf", "query": { "itemNo": this.itemNo } });
        }
      } else if (this.identity === "edit") {
        if (this.saved) {
          this.$router.push({
    ...
    async doSave() {
      ...
      param.specDetail = JSON.stringify(param.specDetail);
      param.specDetail = param.specDetail.replace(/:/g, " ").replace(/[""]/g, "").replace(/[{}]/g, "");
