<template>
  <div class="page-content">
        ...
        <div class="del-form">
            <div class="item-row">
              ...
              <el-form-item label="订单状态">
                <div class="elFont">
                  <div class="elFont">{{getOrderStatus(orderItem.orderStatus)}}</div>
                </div>
              </el-form-item>
              <el-form-item label="订货时间">
               ...
               <el-form-item label="支付状态">
                <div class="elFont">
                  <i-status class="i-status" :type="['warning','success','error'][orderItem.payStatus]" :message="getPayStatus(orderItem.payStatus)" />
                </div>
              </el-form-item>
      ...
      <div class="item-box">
        ...
        <div class="table-main" v-if="open && orderItem.orderItems && orderItem.orderItems.filter(x=>!x.activityItemType).length>0">
          <div class="table">
              <el-table
                :data="orderItem.orderItems && orderItem.orderItems.filter(x=>!x.activityItemType)"
              ...
              >
                ...
                >
                 <template slot-scope="scope">
                    <div>{{calcShouldPay(scope.row)}}</div>
                 </template>
                </el-table-column>
                <el-table-column
                  prop="payAmount"
      ...
      <div class="item-box">
         <div class="actionbar">
            ...
            <div class="select" @click="isShowTwo">
              <span>收起</span>
              <i class="el-icon-arrow-up" v-if="openTwo"></i>
              <i class="el-icon-arrow-down" v-else></i>
            </div>
          </div>
          <div class="table-main" v-if="openTwo && orderItem.orderItems && orderItem.orderItems.filter(x=>x.activityItemType===1).length>0">
            <div class="table">
                <el-table
                  :data="orderItem.orderItems && orderItem.orderItems.filter(x=>x.activityItemType===1)"
                  header-cell-class-name="table-header-item"
                >
                  <el-table-column
                      type="index"
