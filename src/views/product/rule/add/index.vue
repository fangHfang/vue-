<template>
  <div class="page">
    <div style="padding: 15px;font-weight:700">编辑积分规则</div>
    <div class="toolbar-box convert-rule-add-toolbar">
      <div>
        <div class="label spec">积分规则名称</div>
        <div class="content">
          <el-input v-model="toolBar.ruleName" placeholder="请输入规则名称"/>
        </div>
      </div>
      <div>
        <div class="label">积分节点</div>
        <div class="content">
    ...
    <div class="toolbar-box-wrap">
      <div class="toolbar-box activity-detail-toolbar-box">
        ...
        <el-button type="primary" @click="activitySearch">{{ $text.search }}</el-button>
        <el-button type="primary" plain @click="activityReset">{{ $text.reset }}</el-button>
        ...
        <importExport
            ...
            exportFileName="积分规则导出"
            @refreshTableData="getRuleData"
        />
      </div>
    </div>
...
<!--      </div>-->
    ...
    <div class="btn-footer">
      <el-button type="primary" @click="saveRuleData" :loading="btnLoading">保存上架</el-button>
      <el-button plain @click="backList">{{ $text.cancel }}</el-button>
    </div>
    <dialogMaxxisBasicProduct :dialogVisible.sync="dialogVisible" @productCode="addProductCode2Table" mark="itRule"/>
    <el-dialog
            title="产品积分数值"
