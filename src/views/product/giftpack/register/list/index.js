import { pageSearchRegPkg, upShelfRegPkg, downShelfRegPkg } from "@/api/product/giftpack";
export default {
  "name": "seckillList",
  "components": {
  },
  ...
  "methods": {
    ...
    getRemoteTableData() {
      ...
      if (this.toolBar.date && this.toolBar.date.length > 0) {
        params.beginTime = this.toolBar.date[0] + " 00:00:00";
        params.endTime = this.toolBar.date[1] + " 23:59:59";
      }
      if (this.toolBar.name) {
        params.pkgName = this.toolBar.name;
      }
      ...
      pageSearchRegPkg(postData).then(res => {
        if (res.code === 200) {
