...
import { listItemCategoryPage, listCategoryNextLevel, delCategoryItem, transferStatus } from "@/api/product/base/class";
export default {
  ...
  "methods": {
    ...
    refreshRowData(val) {
      if (val === "update") {
        this.listCategoryNextLevel(this.parentRow, "expand");
      } else {
     ...
     * @param {id} val
     */
    relationProduct(categoryNo) {
      this.$router.push("/product/base/group/relation/" + categoryNo);
    },
    /**
     * 分页器每页数量变化处理
     * @param val
    ...
    handleSizeChange(val) {
      ...
      const postData = {
        ...
        "pageSize": this.pagination.size,
        "categoryType": 1,
        "categoryName": this.toolBar.name,
    ...
    */
     ...
     * 调取新建方法
     */
    createCategoryType(categoryName, categoryLevel, categoryNo, row, title) {
      if (categoryLevel === 2) {
        this.$message({
          "message": "分类等级最高2级！",
          "type": "warning"
    ...
    edit(row) {
      ...
      this.dialogVisible = true;
      this.currentRow = row;
      this.parentRow = this.tableData.find(x => x.categoryNo === row.parentNo);
    }
  }
