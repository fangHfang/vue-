...
export default {
  ...
  "methods": {
    ...
    toDetail(promotionNo) {
      this.$router.push({ "path": "/product/activity/combination/detail/" + promotionNo });
    },
    /**
     * 打开配置门店权限
     */
    ...
    edit(item) {
      ...
      this.$confirm("活动" + msg + "，是否确认编辑?", "编辑商品组合活动", {
        "confirmButtonText": "编辑",
        "cancelButtonText": "取消",
        "type": "warning"
      }).then(() => {
        this.$router.push({ "path": "/product/activity/combination/edit/" + item.promotionNo });
      }).catch(() => {});
    },
    /**
     * 打开上架
    ...
    onShelves(item) {
      const msg = [ "未开始", "进行中", "已结束" ][item.promotionStatus];
      this.$confirm("活动" + msg + "，是否继续上架？", "上架商品组合活动", {
    ...
    offShelves(item) {
      const msg = [ "未开始", "进行中", "已结束" ][item.promotionStatus];
      this.$confirm("活动" + msg + "，是否继续下架?", "下架商品组合活动", {
    ...
    copyActivity() {
      ...
      this.$router.push({ "path": "/product/activity/combination/copy/" + multipleSelection[0].promotionNo });
    },
    /**
     * 变更上下架状态
     ...
     */
    addOrEdit() {
      this.$router.push({ "path": "/product/activity/combination/add", "query": { "r": this.$getRandomValue() } });
    },
    /**
     * 复制链接
     */
