<template>
  <div class="page-content" v-loading="detailLoading">
    <h3 class="page-name">{{pageTitle}}
            ...
            <el-form-item label="活动列表图" prop="activeListImg">
             ...
             <el-form-item label="活动介绍海报" prop="activeBanner">
              <el-upload
                  ...
                  :class="form.activeBanner.url?'disabled lage':'lage'"
                 >
                <i class="el-icon-plus"></i>
                 <div slot="file" slot-scope="{file}">
                  <img
        ...
        <h4 class="item-title">商品批次</h4>
          <div class="toolbar-box">
            <div>
              ...
              <div class="content">
                <el-input v-model="toolBar.itemName" clearable placeholder="请输入商品名称"></el-input>
              </div>
            </div>
            <div>
              <div class="label">商品编码</div>
              <div class="content">
                <el-input v-model="toolBar.itemNo" clearable placeholder="请输入商品编码"></el-input>
              </div>
            </div>
            <div>
              <div class="label">商品品牌</div>
              <div class="content">
                <el-input v-model="toolBar.brandName" clearable placeholder="请输入商品品牌"></el-input>
              </div>
            </div>
            <el-button type="primary" @click="filterSearch">{{ $text.search }}</el-button>
            <el-button type="primary" plain @click="filterReset">{{ $text.reset }}</el-button>
        </div>
        <div class="table-main">
            <div class="actionbar" v-if="identity!=='detail'">
              <el-button type="primary" @click="batchBtnLoading = true;$throttle(addBatch,500)" :loading='batchBtnLoading'>添加批次</el-button>
      ...
      <div class="item-box" v-for="(seckill,index) in seckillBatchCreateReqs" :key="index">
        <div class="table-main gray-bg-box">
          <div class="gray-bg-box">
            <div class="del-form">
              <el-form-item :label="'第'+(index+1)+'批次时间'" style="width:400px;" label-width="100px">
                <el-date-picker
                  v-model="seckill.timeRange"
                  size="mini"
                  type="datetimerange"
                  value-format="yyyy-MM-dd HH:mm:ss"
                  range-separator="~"
            ...
            <el-form-item label="选择商品" style="margin:0" label-width="100px" v-if="identity!=='detail'">
              <el-button @click="addProduct(index)" type="primary">添加商品</el-button>
              <el-button type="primary" @click="removeBatch(index)">取消批次</el-button>
            </el-form-item>
          </div>
           <div class="table">
              <el-table
                :data="seckill.seckillCreateReqs"
                header-cell-class-name="table-header-item"
              >
                <el-table-column
                  type="selection"
