...
export default {
  ...
  "methods": {
    ...
    copyActivity() {
      ...
      this.$router.push({ "path": "/product/activity/seckill/copy/" + multipleSelection[0].promotionNo });
    },
    /**
     * 批量删除
     */
    ...
    toDetail(promotionNo) {
      this.$router.push({ "path": "/product/activity/seckill/detail/" + promotionNo });
    },
    /**
     * 打开配置门店权限
     */
    ...
    edit(item) {
      ...
      this.$confirm("活动" + msg + "，是否确认编辑?", "编辑秒杀活动", {
        "confirmButtonText": "编辑",
        "cancelButtonText": "取消",
        "type": "warning"
      }).then(() => {
        this.$router.push({ "path": "/product/activity/seckill/edit/" + item.promotionNo });
      }).catch(() => {});
    },
    /**
     * 打开上架
    ...
    onShelves(item) {
      const msg = [ "未开始", "进行中", "已结束" ][item.promotionStatus];
      this.$confirm("活动" + msg + "，是否继续上架？", "上架秒杀活动", {
    ...
    offShelves(item) {
      const msg = [ "未开始", "进行中", "已结束" ][item.promotionStatus];
      this.$confirm("活动" + msg + "，是否继续下架?", "下架秒杀活动", {
     ...
     */
    addOrEdit() {
      this.$router.push({ "path": "/product/activity/seckill/add", "query": { "r": this.$getRandomValue() } });
    },
