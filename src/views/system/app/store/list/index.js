...
import storeApi from "@/api/system/app/store";
export default {
  ...
  "methods": {
    ...
    async getRemoteStoreTableData() {
      ...
      const res = await storeApi[this.mark + "QueryWhiteList"](this.$jsonFormat(params));
      if (res.code === 200) {
    ...
    listItemVisibilityPage() {
      ...
      storeApi[this.mark + "QueryVisibility"](params).then(res => {
        if (res.code === 200) {
    ...
    getSelectStore(list) {
      ...
      storeApi[this.mark + "CreateWhiteList"](params).then((res) => {
        if (res.code === 200) {
