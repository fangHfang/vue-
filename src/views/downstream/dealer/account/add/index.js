// src/views/downstream/dealer/account/add/index
import { createUserAccount, modifyUserAccount, detailUserAccount, listDealerPage } from "@/api/downstream/dealer/account";

import Config from "../role/index.vue";
export default {
  "name": "DealerAccountManageAdd",
  "components": {
    Config
  },
  data() {
  ...
  "computed": {
    ...
    isModify() {
      return this.$store.state.dealerAccount.accountModify;
    }
  },
  created() {
    this.getListDealerPage();
  },
  mounted() {
    if (this.isModify) {
      this.getUserDetail();
  ...
  "methods": {
     ...
     * @param name 关键字
     */
    getListDealerPage(name = "") {
      const data = {
        "name": name,
        "page.pageNum": 1,
        "page.pageSize": 40
      };
      listDealerPage(this.$jsonFormat(data)).then(res => {
